...
class MapMatcher:
    ...
    def step7_calculate_projections(self):
        ...
        with self.conn.cursor() as cur:
            try:
                # Calculate cumuld_client and geom_prj
                logger.info("Step 7a: Calculating cumuld_client and geom_prj")
                cur.execute("""
                    UPDATE public.image point 
                    SET 
                        cumuld_client = ST_M(ST_LineInterpolatePoint(line.geom_calib, ST_LineLocatePoint(line.geom_calib, point.geom))),
                ...
                """)
                logger.info(f"Updated cumuld_client and geom_prj for {cur.rowcount} images")
                ...
                cur.execute("""
                    ...
                    SET seg_prj = ST_Force2D(
                        st_geometryN(
                            ST_LocateBetween(t2.geom_calib, t1.cumuld_client-1.0, t1.cumuld_client+1.0), 1
                        )
                    )
                    FROM client.troncon_client t2 
                    WHERE t1.id_tronc = t2.id_tronc AND t1.cumuld_client IS NOT NULL
                """)
    ...
    def step8_handle_perpendicular_cases(self):
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute("""
                UPDATE public.image 
                SET id_tronc = NULL, prj_quality = NULL, cumuld_client = NULL, geom_prj = NULL, seg_prj = NULL
        ...
        with self.conn.cursor() as cur:
            try:
                ...
                cur.execute("""
                    ...
                    FROM client.troncon_client t2 
                    WHERE (t1.d_angle_seg BETWEEN 45.0 AND 135.0) AND t1.id_tronc = t2.id_tronc AND t1.cumuld_client IS NOT NULL
                """)
